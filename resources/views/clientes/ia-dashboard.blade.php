...
{{-- VALIDACIÓN DOCUMENTACIÓN — MES ACTUAL --}}
<div class="ia-card">
    <div class="ia-card-head">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--purple)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><path d="M9 15l2 2 4-4"/></svg>
        <h3>Validación de documentos (mes actual)</h3>
    </div>
    <div class="ia-card-body">
        <p class="ia-desc">La IA arma un mensaje de validación con base en tu expediente y el checklist del <strong>mes en curso ({{ now()->format('m/Y') }})</strong>: documentos faltantes, pendientes de revisión o que deben actualizarse según política del portal.</p>
        <form method="POST" action="{{ route('clientes.ia.documentacion') }}">
            @csrf
        ...
        <div class="ia-result">
            ...
            @if($resultadoDocumentacion['analisis']['success'] ?? false)
                <div class="ia-response">{!! \Illuminate\Support\Str::markdown($resultadoDocumentacion['analisis']['content']) !!}</div>
            @else
