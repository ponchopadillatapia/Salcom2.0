...
<style>
    ...
    .inv-kpi{background:var(--white);border:2px solid var(--border-light);border-radius:14px;padding:22px;text-align:center;position:relative;overflow:hidden;cursor:pointer;transition:var(--transition);text-decoration:none;color:inherit;display:block}
    ...
    .table-head{padding:16px 22px;font-size:13px;font-weight:700;color:var(--gray-text);background:var(--gray-soft);border-bottom:1px solid var(--border-light);display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px}
    .table-head .filter-label{font-size:12px;color:var(--purple);font-weight:600}
    .table-head .page-info{font-size:12px;color:var(--gray-muted);font-weight:500}
    ...
    .pagination-wrap{padding:16px 22px;display:flex;justify-content:center;border-top:1px solid var(--border-light)}
...
@php
    $filtroLabels = ['all' => 'Todos', 'out' => 'Agotados', 'low' => 'Stock Bajo', 'ok' => 'Stock OK'];
    $filtroActual = $filtro ?? 'all';
@endphp

<div class="inv-kpis anim">
    <a href="{{ route('admin.inventario', ['stock' => 'out']) }}" class="inv-kpi {{ $filtroActual === 'out' ? 'active-filter' : '' }}">
    ...
    <a href="{{ route('admin.inventario', ['stock' => 'low']) }}" class="inv-kpi {{ $filtroActual === 'low' ? 'active-filter' : '' }}">
    ...
    <a href="{{ route('admin.inventario', ['stock' => 'ok']) }}" class="inv-kpi {{ $filtroActual === 'ok' ? 'active-filter' : '' }}">
    ...
    <a href="{{ route('admin.inventario') }}" class="inv-kpi {{ $filtroActual === 'all' ? 'active-filter' : '' }}">
        <div class="bar" style="background:var(--purple)"></div>
        <div class="inv-kpi-val" style="color:var(--purple)">{{ $totalProductos }}</div>
...
<div class="table-card">
    <div class="table-head">
        ...
        <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap;">
            <span class="filter-label">Mostrando: {{ $filtroLabels[$filtroActual] ?? 'Todos' }}</span>
            @if($productos->total() > 0)
                <span class="page-info">{{ $productos->firstItem() }}–{{ $productos->lastItem() }} de {{ $productos->total() }}</span>
            @endif
        </div>
    </div>
    @if($productos->count())
    <table class="tbl">
        ...
        @foreach($productos as $p)
            @php
                ...
                $provUser = $p->proveedor_nombre ? ($proveedoresContacto[$p->proveedor_nombre] ?? null) : null;
    ...
    @endif
    @else
    <div class="empty-state">No hay productos con el filtro seleccionado.</div>
    @endif
</div>
