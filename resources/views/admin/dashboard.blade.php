...
@section('content')
@php
    // Variación ventas: mes anterior completo vs penúltimo mes (evita mes en curso incompleto)
    ...
    $ventasVarPct = ($penultimoMesData && $penultimoMesData['monto'] > 0)
        ? round((($mesAnteriorData['monto'] - $penultimoMesData['monto']) / $penultimoMesData['monto']) * 100)
        : 0;
    $pedidosVarPct = ($penultimoMesData && $penultimoMesData['total'] > 0)
...
<div class="pp-wrap">
    ...
    <div class="pp-grid-2" style="grid-template-columns:repeat(4,minmax(0,1fr));align-items:stretch;">
        ...
        <div class="pp-card" style="height:100%;display:flex;flex-direction:column;">
            <h4>Negocio</h4>
            <div class="pp-negocio-row"><div class="pp-negocio-label">Ventas totales</div><div class="pp-negocio-value" style="color:var(--green);font-size:20px;">${{ number_format($montoPedidos, 0) }} @if($ventasVarPct != 0)<span class="pp-variation {{ $ventasVarPct > 0 ? 'pp-variation-up' : 'pp-variation-down' }}" style="font-size:14px;">{{ $ventasVarPct > 0 ? '↑' : '↓' }} {{ $ventasVarPct > 0 ? '+' : '' }}{{ $ventasVarPct }}%</span>@endif</div></div>
            <div class="pp-negocio-row"><div class="pp-negocio-label">Pedidos</div><div class="pp-negocio-value" style="font-size:20px;">{{ $totalPedidos }} @if($pedidosVarPct != 0)<span class="pp-variation {{ $pedidosVarPct > 0 ? 'pp-variation-up' : 'pp-variation-down' }}" style="font-size:14px;">{{ $pedidosVarPct > 0 ? '↑' : '↓' }} {{ $pedidosVarPct > 0 ? '+' : '' }}{{ $pedidosVarPct }}%</span>@endif</div></div>
    ...
    <div class="pp-quick-grid">
        ...
        <a href="{{ route('admin.clientes') }}" class="pp-card pp-quick-card"><div class="pp-quick-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--purple)" stroke-width="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></div><div><div class="pp-quick-title">Clientes</div><div class="pp-quick-sub">Gestión</div></div></a>
        ...
        <a href="{{ route('admin.negocio') }}" class="pp-card pp-quick-card"><div class="pp-quick-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--purple)" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg></div><div><div class="pp-quick-title">Negocio</div><div class="pp-quick-sub">Ventas y finanzas</div></div></a>
        <a href="{{ route('admin.otif') }}" class="pp-card pp-quick-card"><div class="pp-quick-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--purple)" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></div><div><div class="pp-quick-title">OTIF</div><div class="pp-quick-sub">Cumplimiento</div></div></a>
        <a href="{{ route('admin.encuestas') }}" class="pp-card pp-quick-card"><div class="pp-quick-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--purple)" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></div><div><div class="pp-quick-title">Encuestas</div><div class="pp-quick-sub">Satisfacción</div></div></a>
