...
<div class="hero-band">
    <h1>Alta Facturas</h1>
    <p>Carga facturas de proveedores · PDF + XML · valida y luego sube</p>
</div>
@endsection
...
<form method="POST" action="{{ route('admin.facturas.alta.validar') }}" enctype="multipart/form-data" id="formFiscal">
    @csrf
    ...
    <div class="id-card">
        ...
        <p class="card-desc">Selecciona el proveedor, adjunta PDF + XML (OC opcional). Primero valida; si todo está bien, pulsa Subir para registrar.</p>
        ...
        <div class="form-row" style="margin-bottom:20px;">
            <div class="form-group">
                ...
                <select name="proveedor_id" id="proveedor_id" required onchange="if(this.value){ window.location='{{ route('admin.facturas.alta') }}?proveedor_id='+this.value; }">
                    <option value="">— Selecciona un proveedor —</option>
                    @foreach($proveedores as $p)
                        <option value="{{ $p->id }}" {{ (string) old('proveedor_id', $proveedorId ?? '') === (string) $p->id ? 'selected' : '' }}>
                            {{ $p->nombre ?: 'Sin nombre' }}@if($p->id_proveedor) · {{ $p->id_proveedor }}@endif
                        </option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="form-actions">
            ...
            <button type="submit" class="btn-outline" id="btnValidar" formaction="{{ route('admin.facturas.alta.validar') }}">Validar</button>
            <button type="submit" class="btn-submit" id="btnSubir" formaction="{{ route('admin.facturas.alta.subir') }}" formnovalidate {{ !empty($puedeSubir) ? '' : 'disabled' }}>Subir</button>
        </div>
    </div>
</form>
...
<div class="card">
    <div class="card-head">
        <h3>Facturas recientes {{ $proveedor ? '· '.($proveedor->nombre ?: $proveedor->id_proveedor) : '' }}</h3>
        <span style="font-size:12px;color:var(--gray-muted);">{{ $facturas->count() }} subidas</span>
    </div>
    @if(! $proveedor)
        <div class="empty-state">Selecciona un proveedor para ver sus facturas recientes.</div>
    @elseif($facturas->isEmpty())
        <div class="empty-state">Este proveedor aún no tiene facturas registradas.</div>
    @else
    ...
    if (form && btnValidar) {
        form.addEventListener('submit', function (e) {
            ...
            if (proveedor && !proveedor.value) {
                e.preventDefault();
                alert('Selecciona el proveedor al que pertenece la factura.');
                return;
            }
