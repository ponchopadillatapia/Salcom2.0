...
class AdminPanelController extends Controller
{
    ...
    /**
     * Alta de facturas (copia del flujo de proveedor) para admin.
     */
    public function mostrarAltaFacturas(Request $request)
    {
        $proveedores = ProveedorUser::query()
            ->where('activo', true)
            ->orderBy('nombre')
            ->get(['id', 'nombre', 'id_proveedor', 'rfc', 'datos_identificacion']);

        $proveedorId = (int) ($request->input('proveedor_id') ?: (session('admin_fiscal_pendiente.proveedor_id') ?? 0));
        $proveedor = $proveedorId > 0 ? ProveedorUser::find($proveedorId) : null;

        $codigo = $proveedor?->id_proveedor;
        $facturas = Factura::query()
            ->when($codigo, fn ($q) => $q->where('codigo_proveedor', $codigo))
            ->when(! $codigo, fn ($q) => $q->whereRaw('1 = 0'))
            ->where('estatus', '!=', 'rechazada')
            ->orderByDesc('created_at')
            ->limit(15)
            ->get();

        $pendiente = $this->adminFiscalPendienteVigente($proveedor);
        ...
        return view('admin.alta-facturas', compact(
            'proveedores',
            'proveedor',
            'proveedorId',
            'facturas',
    ...
    public function validarAltaFactura(Request $request, AltaFacturaValidationService $validator)
    {
        $request->validate([
            ...
            'proveedor_id.required' => 'Selecciona el proveedor al que pertenece la factura.',
            'proveedor_id.exists' => 'El proveedor seleccionado no es válido.',
        ]);

        $proveedor = ProveedorUser::find($request->input('proveedor_id'));
        if (! $proveedor) {
            return back()->withErrors(['proveedor_id' => 'Proveedor no válido.']);
        }

        $prev = $this->adminFiscalPendienteVigente($proveedor);
        ...
        $this->guardarAdminFiscalTemporal(
            $proveedor,
            (string) $pdfContent,
            (string) $xmlContent,
        ...
        $rfcProveedor = $this->rfcProveedorAdmin($proveedor);
        ...
        $this->actualizarAdminFiscalPendiente([
    ...
    public function altaFactura(Request $request)
    {
        ...
        try {
            $this->registrarFacturaAdminDesdePendiente($proveedor, $pendiente);
        } catch (\InvalidArgumentException $e) {
            $this->limpiarAdminFiscalPendiente();
            ...
            $this->limpiarAdminFiscalPendiente();
            Log::error('[AdminAltaFactura] Error al registrar: '.$e->getMessage());

            return back()->withInput()->withErrors(['archivo' => 'Error al guardar la factura. Intenta de nuevo.']);
        }

        $this->limpiarAdminFiscalPendiente();

        $estatus = $resultado['estatus'] ?? 'aprobada';
        $mensaje = 'Factura registrada correctamente a nombre de '
            .($proveedor->nombre ?: $proveedor->id_proveedor)
            .'. Queda pendiente de revisión contable.';

        return redirect()
            ->route('admin.facturas.alta', ['proveedor_id' => $proveedor->id])
            ->with('fiscal_resultado', [
                'aprobado' => true,
                'estatus' => $estatus,
                'mensaje' => $mensaje,
                'errores' => [],
     ...
     */
    private function registrarFacturaAdminDesdePendiente(ProveedorUser $proveedor, array $pendiente): Factura
    {
        ...
        $codigoProv = $proveedor->id_proveedor ?: ('P'.$proveedor->id);
        ...
        try {
            ...
            app(AlertEngineService::class)->crearAlerta([
                ...
                'contenido' => "Folio {$folioCfdi} · \${$totalFmt} · pendiente de pago (alta admin)",
                'datos' => [
     ...
     */
    private function adminFiscalPendienteVigente(?ProveedorUser $proveedor): ?array
    {
        if (! $proveedor) {
            return null;
        }

        $pendiente = session('admin_fiscal_pendiente');
    ...
    private function guardarAdminFiscalTemporal(
        ...
        $tempDir = 'temp-fiscal-admin/'.$proveedor->id.'/'.$token;
        $disk = Storage::disk('local');
        ...
        session(['admin_fiscal_pendiente' => [
            'token' => $token,
     ...
     */
    private function actualizarAdminFiscalPendiente(array $campos): void
    {
        $pendiente = session('admin_fiscal_pendiente');
        if (! is_array($pendiente)) {
            return;
        ...
        session(['admin_fiscal_pendiente' => array_merge($pendiente, $campos)]);
    }

    private function limpiarAdminFiscalPendiente(): void
    {
        $pendiente = session('admin_fiscal_pendiente');
        ...
        session()->forget('admin_fiscal_pendiente');
    }

    private function rfcProveedorAdmin(?ProveedorUser $proveedor): ?string
    {
        if (! $proveedor) {
            return null;
        }
        ...
        return $rfc ? strtoupper(trim((string) $rfc)) : null;
    }
